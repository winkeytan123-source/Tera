...
import tera
from tera import Pico, SessionStore, WorkspaceContext, build_agent, build_arg_parser, build_welcome, main
...
def test_public_api_exports_current_names_only():
    ...
    assert not hasattr(tera, "MiniAgent")
    assert "MiniAgent" not in tera.__all__
...
def test_lightweight_package_split_uses_package_paths_without_legacy_shims():
    from tera.evaluation.evaluator import BenchmarkEvaluator
    from tera.evaluation.metrics import run_context_ablation_v2
    from tera.features.memory import LayeredMemory
    from tera.providers.clients import FakeModelClient as ProviderFakeModelClient
    ...
    for legacy_module in ("evaluator.py", "metrics.py", "models.py", "memory.py"):
        assert not (Path("tera") / legacy_module).exists()


def test_packaging_discovers_tera_subpackages():
    pyproject_text = Path("pyproject.toml").read_text(encoding="utf-8")

    assert "[tool.setuptools.packages.find]" in pyproject_text
    assert 'include = ["tera*"]' in pyproject_text
