...
from tera import FakeModelClient, Pico, SessionStore, WorkspaceContext
from tera import cli as tera_cli
from tera.task_state import TaskState
...
def test_cli_build_agent_loads_project_env_secrets_before_redaction_setup(tmp_path):
    ...
    with patch.dict(os.environ, {}, clear=True), patch("tera.cli.AnthropicCompatibleModelClient", DummyModelClient):
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        ...
        clear=True,
    ), patch("tera.cli.OllamaModelClient", DummyModelClient):
        args = pico_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = pico_cli.build_agent(args)
...
def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    ...
    with patch("tera.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "tera.runtime"

    with patch("tera.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
